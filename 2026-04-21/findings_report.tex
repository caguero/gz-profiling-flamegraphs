...
The study follows two phases. The \textbf{discovery phase} uses CPU flamegraphs to identify where time goes across the full stack and prioritize optimization targets. The \textbf{validation phase} (Section~\ref{validating-optimizations}) defines a methodology for measuring the before/after impact of each optimization using \texttt{GZ\_PROFILE} (Remotery) instrumentation, which provides deterministic per-step timing for named code scopes.
...
For the validation phase (measuring before/after impact of specific optimizations), \texttt{ENABLE\_PROFILER=ON} should be used. This activates the Remotery scopes that provide per-step timing for each named code block, enabling precise comparison between baseline and optimized implementations. See Section~\ref{validating-optimizations} for the recommended validation workflow.
...
profiling/
  ...
  scripts/
    ...
    capture_all.sh                 # Orchestrates all captures
\end{verbatim}
...
\item \texttt{*.folded} --- collapsed stack traces in text format (one line per unique stack, with sample count). Input for \texttt{flamegraph.pl}. Also useful for CLI analysis with \texttt{awk}/\texttt{sort}.
...
gz-profiling/
  ...
  2026-04-21/                 # This benchmark run
    runtime/                  #   Interactive runtime flamegraph SVGs
    loading/                  #   Interactive loading flamegraph SVGs
...
\begin{enumerate}
\item \textbf{Baseline}: Run the benchmark world with the current code. Record the Remotery per-step timing for the relevant scope.
...
\hypertarget{instrumentation-gaps}{%
\section{Instrumentation Gaps}\label{instrumentation-gaps}}

\begin{itemize}
\tightlist
\item
  \textbf{\texttt{{[}libdart.so.6.13.2{]}} unresolved}: 1.5-10.2\%
  across worlds. Rebuilding DART with \texttt{-g} would expose internal
  hotspots. Especially important for 3k\_shapes\_dynamic where 10.2\% is
  opaque.
\item
  \textbf{\texttt{{[}unknown{]}} frames}: 10-35\% in several worlds ---
  kernel stacks and stripped system libraries.
  \texttt{-\/-call-graph\ dwarf} or installing
  \texttt{linux-image-dbgsym} would resolve some.
\item
  \textbf{\texttt{void}/\texttt{operator}/\texttt{unsigned} demangling
  artifacts}: 12-35\% --- FlameGraph's stack-folding splits on
  \texttt{;} inside demangled C++ template signatures. Not real
  functions; their cost belongs to the parent frame.
\item
  \textbf{Ogre2 internals}: Scene-graph traversal is 22\% in
  sensors\_demo but Ogre2 doesn't have debug symbols. Rebuilding Ogre2
  with \texttt{-g} would reveal whether the cost is in dirty-flag
  checking, matrix multiplication, or memory layout.
\end{itemize}
...
\hypertarget{a-flamegraph-svg-locations}{%
\subsection{A: Flamegraph SVG
Locations}\label{a-flamegraph-svg-locations}}

\textbf{Runtime} (in \texttt{profiling/captures/flamegraphs/runtime/}):

\begin{verbatim}
3k_shapes_static.svg
3k_shapes_dynamic.svg
sensors_nonrendering.svg
jetty_headless.svg
gpu_lidar.svg
sensors_demo.svg
\end{verbatim}

\textbf{Loading} (in \texttt{profiling/captures/flamegraphs/loading/}):

\begin{verbatim}
3k_shapes_static_loading.svg
3k_shapes_dynamic_loading.svg
sensors_nonrendering_loading.svg
jetty_loading.svg
gpu_lidar_loading.svg
sensors_demo_loading.svg
\end{verbatim}

\hypertarget{b-gz_profile-instrumentation-added}{%
\subsection{B: GZ\_PROFILE Instrumentation
Added}\label{b-gz_profile-instrumentation-added}}

17 profile points added to gz-physics/dartsim: -
\texttt{SimulationFeatures.cc}: 7 points (WorldForwardStep + 6 new
sub-phases) - \texttt{GzCollisionDetector.cc}: 4 points (ODE + Bullet
collide wrappers) - \texttt{SDFFeatures.cc}: 5 points (ConstructSdf*
methods) - \texttt{KinematicsFeatures.cc}: 1 point
(FrameDataRelativeToWorld)

\hypertarget{c-loading-wall-clock-times}{%
\subsection{C: Loading Wall-Clock
Times}\label{c-loading-wall-clock-times}}
...
\bottomrule\noalign{}
\endlastfoot
3k\_shapes\_static & 26.0s \\
3k\_shapes\_dynamic & 26.6s \\
jetty & 7.8s \\
sensors\_nonrendering & 2.4s \\
gpu\_lidar & 0.06s \\
sensors\_demo & 0.03s \\
\end{longtable}
