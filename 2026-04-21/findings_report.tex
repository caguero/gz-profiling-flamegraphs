...
\item
  \textbf{gz-physics}: source build (main branch)
\item
  \textbf{DART}: system-packaged libdart 6.13.2 (no debug symbols)
\item
  \textbf{perf}: 6.17.13
\item
  \textbf{FlameGraph}: Brendan Gregg's scripts (\url{https://github.com/brendangregg/FlameGraph})
\end{itemize}
...
Replace \texttt{<GZ\_WS>} with the path to your Gazebo workspace (e.g., \texttt{\textasciitilde{}/rotary\_ws}).

\subsection{Benchmark Worlds}

All benchmark worlds are in the repository's \texttt{worlds/} directory with
\texttt{real\_time\_factor} set to \texttt{0} to saturate CPU.
\texttt{3k\_shapes\_static.sdf} additionally has
\texttt{static} set to \texttt{true} on all 3004 models.
Sensor worlds have companion \texttt{.topics} files listing topics for subscriber setup.

\subsection{Running Captures}

From the \texttt{gz-profiling} repository directory:

\begin{verbatim}
# Capture all worlds (runtime + loading flamegraphs)
...
This adds a second optimization lever beyond algorithmic fixes: \textbf{data structure layout}. Switching from \texttt{unordered\_map} to contiguous storage (e.g., a flat vector sorted by entity ID) would dramatically improve cache performance by enabling sequential memory access instead of pointer-chasing through heap-allocated hash-map nodes.
